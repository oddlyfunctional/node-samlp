...
function buildSamlResponse(options) {
  var SAMLResponse = templates.samlresponse({
    id:             options.id || '_' + utils.generateUniqueID(),
    instant:        options.instant || utils.generateInstant(),
...
function createResponse(user, samlRequest, options, next) {
  options = Object.assign(getOptionDefaults(samlRequest), options);
  getSamlResponse(options, user, next);

  function getOptionDefaults(samlRequest) {
    const options = {
    ...
    if (!options.audience){
      ...
      if (issuer && issuer.length > 0)
        options.audience = issuer[0].textContent;
    }

    var id = samlRequest.id;
...
function sendResponse(SAMLResponse, postUrl, RelayState, res) {
...
module.exports.auth = function(options) {
  ...
  return function (req, res, next) {
    ...
    // TODO: Verify that these are not used
...
module.exports.sendError = function (res, postUrl, options) {
  // https://docs.oasis-open.org/security/saml/v2.0/saml-core-2.0-os.pdf
  return function renderResponse(error) {
    error = error || {};
    const opts = Object.assign({}, options);
    opts.samlStatusCode = error.code || constants.STATUS.RESPONDER;
    opts.samlStatusMessage = error.message;

    var SAMLResponse = buildSamlResponse(opts);
    var response = new Buffer(SAMLResponse);
...
module.exports.sendErrorMiddleware = function (options) {
  // https://docs.oasis-open.org/security/saml/v2.0/saml-core-2.0-os.pdf
